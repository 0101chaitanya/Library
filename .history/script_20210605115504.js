...
submitBtn.addEventListener('click',
    function() {
        let obj = new setBookObj();
        //inputElem[0].value, inputElem[1].value, inputElem[2].value, inputElem[3].checked

        let inputElems = Array.from(form.elements);
        inputElems.forEach((item, i) => {
            if (item.tagName == 'input') {
                Object.values(obj)[i] = item.value;
            }
            console.log(obj);
...
var retrivedObj = JSON.parse(localStorage.getItem('obj'));
