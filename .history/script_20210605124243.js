...
submitBtn.addEventListener('click',
    function() {
        // let obj = 
        window[`obj` + i] = new setBookObj(form.elements[0].value, form.elements[1].value, form.elements[2].value, form.elements[3].checked);
        console.log(obj0);

        objArray.splice(i, 0, window[`obj` + i]);
        //    console.log(objArray);
        // objArray.push();
        //        objArray.splice(i, 0, obj);
        localStorage.setItem(`objArray`, JSON.stringify(window[`obj` + i]));
        i++;
        form.submit();
...
let divArray = JSON.parse(localStorage.getItem(`objArray`));

console.log(divArray);
/* 
divArray.forEach((item, j) => {
