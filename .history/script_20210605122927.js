...
submitBtn.addEventListener('click',
        function() {
            let obj = window[`obj` + i] = new setBookObj(form.elements[0].value, form.elements[1].value, form.elements[2].value, form.elements[3].checked);
            console.log(obj0);

            // objArray.push(obj);

            //        objArray.splice(i, 0, obj);
            localStorage.setItem('obj' + i, JSON.stringify(obj); i++; form.submit();


            }); //console.log(ObjArray.length);


        let divArray = [JSON.parse(localStorage.getItem('obj' + i))];
